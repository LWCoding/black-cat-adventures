...
The scene name string in `SpaceData.SceneToLoad` must match the filename (without `.unity`) exactly — Unity loads scenes by name via `SceneManager.LoadScene(name)`. Choose a clear, PascalCase name (e.g. `Shop`, `RestSite`, `TreasureRoom`).

## 2. Write the minimal .unity scene file
...
Camera:
  ...
  m_Anamorphism: 0
  m_SensorSize: {x: 23.999998, y: 13.5}
  m_LensShift: {x: 0, y: 0}
...
MonoBehaviour:
  ...
  m_Enabled: 1
  m_EditorHideFlags: 0
  m_Script: {fileID: 11500000, guid: <YOUR SCENE INITIALIZER SCRIPT GUID>, type: 3}
  m_Name: 
  m_EditorClassIdentifier: 
```

> **Important:** The `m_Script` GUID in the `MonoBehaviour` section must reference the scene's bootstrap/initializer C# script. If you haven't written that script yet, write it first (following the pattern in [space.md](space.md) section 4), then read its `.meta` for the GUID before writing the scene file.
>
...
Name it `<SceneName>Spawner` or `<SceneName>Initializer` (not a `*Manager` name unless it derives from `Singleton<T>` — see CLAUDE.md).
