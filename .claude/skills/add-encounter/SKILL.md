...
Like all assets here, everything references C# scripts and other assets by **GUID**. A wrong GUID produces a silent "missing reference" in Unity with no compile error. Read [../add-game-content/references/asset-mechanics.md](../add-game-content/references/asset-mechanics.md) before writing anything if the GUID/`.meta` mechanics are unclear, then read [references/encounter.md](references/encounter.md) for the exact YAML templates.
...
For every enemy in the list that doesn't exist yet, create its `EnemyData` `.asset` + sibling `.meta` by following **steps 1–2 of** [../add-game-content/references/enemy.md](../add-game-content/references/enemy.md) (resolve sprites, write the `EnemyData` `.asset`, hand-write its `.meta` with a fresh GUID).

**Skip steps 3–4 of that reference** (duplicate `Enemy.prefab`, drag into scene, wire `_nextBattleObject`). The encounter system spawns from one shared prefab and chains enemies in code, so per-enemy prefabs and scene placement are not needed.

Record each enemy's `EnemyData` `.asset` GUID — you reference it in the encounter next. For **existing** enemies, read their `Assets/Resources/ScriptableObjects/Characters/<Name>.asset.meta` to get the GUID (never guess it).
...
## 3. Write the Encounter `.asset.meta`

Hand-write the sibling `.meta` using `NativeFormatImporter` with `mainObjectFileID: 11400000` and a **fresh** GUID (distinct from every enemy and the script). See "Writing a `.meta` for a brand-new `.asset` file" in [../add-game-content/references/asset-mechanics.md](../add-game-content/references/asset-mechanics.md). Save this GUID — the database references the encounter by it.

## 4. Register in `_EncounterDatabase.asset`

Read `Assets/Resources/ScriptableObjects/Encounters/_EncounterDatabase.asset` and edit it **in place** (never invent a GUID for the database asset itself):
- **Tutorial encounter** → set `TutorialEncounter: {fileID: 11400000, guid: <encounter guid from step 3>, type: 2}`.
- **Normal encounter** → append to `Entries`:

```yaml
- Encounter: {fileID: 11400000, guid: <encounter guid from step 3>, type: 2}
  Weight: <weight from step 0>
  MinEncountersCompleted: <min from step 0>
```
...
2. Select the new `<EncounterName>.asset`. In the Inspector confirm: no "Missing Script" warning, `EncounterId` is correct, and every `EnemySpawn`'s `EnemyData` is populated (not "None"/"Missing") and in the intended order.
3. For each newly created enemy, open its `EnemyData` `.asset` and confirm all sprite/attack references are populated (same checks as the enemy skill).
4. Select `_EncounterDatabase.asset` and confirm the new encounter appears — either as a new `Entries` row with the right `Weight`/`MinEncountersCompleted`, or in the `TutorialEncounter` slot.
